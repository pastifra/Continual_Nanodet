...
parser.add_argument('--alpha', type=float, help='Weight for class output distillation loss', default=1.0)
parser.add_argument('--beta', type=float, help='Weight for intermediate distillation loss', default=1.0)
...
for task in range (0, total_tasks):
    ...
    if task == 0:
        ...
        TrainTask = DistTrainingTask(cfg, args.alpha, args.beta,evaluator)
        ...
        if "load_model" in cfg.schedule:
            ...
            #Randomly initialize the weights of the heads for the new classes predictions
            '''
            for i in range(0, 4):
                init.kaiming_uniform_(TrainTask.model.head.gfl_cls[0].weight[14+task:20], nonlinearity='relu')
                init.constant_(TrainTask.model.head.gfl_cls[i].bias[14+task:20], -4.5950)
            '''
