...
@weighted_loss
def dist_quality_focal_loss(pred, target, beta=2.0, class_x=None):
    r"""Quality Focal Loss (QFL) is from `Generalized Focal Loss: Learning
    Qualified and Distributed Bounding Boxes for Dense Object Detection
    <https://arxiv.org/abs/2006.04388>`_.
    ...
    Args:
        ...
        beta (float): The beta parameter for calculating the modulating factor.
            Defaults to 2.0.
        class_x (int): The class index for which the loss is computed.
...
class DistQualityFocalLoss(nn.Module):
    r"""Quality Focal Loss (QFL) is a variant of `Generalized Focal Loss:
    Learning Qualified and Distributed Bounding Boxes for Dense Object
    Detection <https://arxiv.org/abs/2006.04388>`_.
    ...
    def __init__(self, use_sigmoid=True, beta=2.0, reduction="mean", loss_weight=1.0, class_x=None):
        super(DistQualityFocalLoss, self).__init__()
        assert use_sigmoid is True, "Only sigmoid in QFL supported now."
        self.use_sigmoid = use_sigmoid
        self.beta = beta
        self.reduction = reduction
    ...
    def forward(
        ...
        reduction = reduction_override if reduction_override else self.reduction
        if self.use_sigmoid:
            loss_cls = self.loss_weight * dist_quality_focal_loss(
                pred,
                target,
                weight,
                beta=self.beta,
